...
class Packager(object):
    """The base packager factory for creating and retrieving packaged
    code/test pairs.
    Contains a staticmethod to create unit key based on question name.
    The Packager will hash the code and tests into 2 seperate folders
    with unique names.
    ...
    def parse_testcases(bundle):
        """parses the yaml of the test file and returns
        a list of test input output
        """
    ...
    def bundle(self, question_name, code_str, test_str, version=1):
        """makes a bundle that contains the correct metadata
        and saveds the strings into the correct key
        """

        key = Packager.generate_key_with_versioning(question_name, version)
        updated_test_str = self.__update_file_contents(question_name,
                                                       version,
                                                       test_str)
    ...
    def __construct_meta_data(self, question_name, version):
        return '#QUESTION = {}\n#VERSION = {}\n#KEY = {}\n'''.format(
               question_name,
               version,
               Packager.generate_key_with_versioning(question_name, version))

    def __update_file_contents(self, question_name, version, file_str):
        updated_str = \
            self.__construct_meta_data(question_name, version) + file_str
        return updated_str
    ...
    def get_bundle(self, key):
        ...
        if not (
            os.path.exists(os.path.join(code_folder, key)) and
                os.path.exists(os.path.join(test_folder, key))):
...
class PythonPackager(Packager):
    ...
    def execute(self, bundle):
        """Given a key, locate the bundle in the storage system
        and execute it."""
        ...
        for testcase, test_io in test_cases_dict.items():
            try:
                executor = Executor()
                execution_response[testcase] = \
                    executor.execute(bundle.code_file_path, test_io)
            except Exception:
                raise PackagerException(
                    "I dun goofed during execution of testcase: {}"
                    .format(testcase))
